var ImageBox = function (eid, images, mode) {
  ...
  if (this.buttonsActive) {
    ...
    ImageBox.addEListener(
      document.getElementById(this.eid + "_previous"),
      "click",
      this.clickedPrevious,
    );
    ImageBox.addEListener(
      document.getElementById(this.eid + "_next"),
      "click",
      this.clickedNext,
    );
  ...
  if (ImageBox.imageBoxes.length == 1) {
    // first imagebox
...
ImageBox.createAll = function (startElement) {
  // create in mode 2
  ...
  if (e) {
    ...
    for (var i = 0; i < tags.length; i++) {
      if (
        ImageBox.ElementHasClass(tags[i], "imagebox_thumbnails") ||
        ImageBox.ElementHasClass(tags[i], "galerie")
      ) {
        ImageBox.imagebox_counter++;
        tags[i].setAttribute(
          "id",
          "imagebox_" + ImageBox.imagebox_counter + "_thumbs",
        );
        var newImageBox = ImageBox.create(
          "imagebox_" + ImageBox.imagebox_counter,
          "imagebox_" + ImageBox.imagebox_counter + "_thumbs",
        );
      }
    }
...
ImageBox.create = function (eid, eidThumbs, mode) {
  // create in mode 2 (or 1 --- if mode == 1)
  var images_dom = [];
  var images_names = [];
    ...
    for (var i = 0; i < img_tags.length; i++) {
      ...
      // * /
        ...
        if (matched !== false) {
          ...
          if (matched == 1) {
            images_names.push(
              img_tags[i].src.replace(/&size=1$/, matched_replace_by),
            );
          } else if (matched == 2) {
            images_names.push(
              img_tags[i].src.replace(/&size=2$/, matched_replace_by),
            );
          } else if (matched == 3) {
            images_names.push(
              img_tags[i].src.replace(/&size=3$/, matched_replace_by),
            );
          } else if (matched == 4) {
            images_names.push(
              img_tags[i].src.replace(/&size=4$/, matched_replace_by),
            );
          }
          image_counter++;
        }
...
ImageBox.createFromList = function (eid, images) {
  // create in mode 1
...
ImageBox.hideDisabler = function () {
  ...
  ImageBox.pushOrChangeFaderImageBox(
    "ib_detail_close",
    -1 /* -ImageBox.fade_delta_out */,
  );
...
ImageBox.pushOrChangeFaderImageBox = function (eid, fade_delta) {
  ...
  if (idx < 0) {
    ...
    ImageBox.faders.push([
      eid,
      fade_delta < 0 ? ImageBox.fade_full : 0,
      fade_delta,
      0,
      false,
    ]);
...
ImageBox.prototype = {
  ...
  toggleSlideshow: function (immediately) {
    ...
    if (this.slideshowEnabled) {
      ...
      document.getElementById(this.eid + "_slideshow").src = document
        .getElementById(this.eid + "_slideshow")
        .src.replace(/button_play.png$/, "button_pause.png");
    } else {
      document.getElementById(this.eid + "_slideshow").src = document
        .getElementById(this.eid + "_slideshow")
        .src.replace(/button_pause.png$/, "button_play.png");
    }
  },
  ...
  loadImage: function () {
    ...
    if (this.images_titles.length > 0) {
      var divXtitle = document.getElementById(
        this.eid + "_div_" + (activeImgTag2 + 1) + "_title",
      );
      var divXtitle_shadow = document.getElementById(
        this.eid + "_div_" + (activeImgTag2 + 1) + "_title_shadow",
      );
    ...
    var divXimg = document.getElementById(
      this.eid + "_div_img_" + (activeImgTag2 + 1),
    );
  ...
  imageLoaded: function (e) {
    if (this.activeImgTag >= 0) {
      ImageBox.pushOrChangeFaderImageBox(
        this.eid + "_div_" + (this.activeImgTag + 1),
        -ImageBox.fade_delta_out / 2,
      );
      document.getElementById(
        this.eid + "_div_" + (this.activeImgTag + 1),
      ).style.zIndex = 2;
      document.getElementById(
        this.eid + "_div_" + (((this.activeImgTag + 1) % 2) + 1),
      ).style.zIndex = 1;
    }
    this.activeImgTag = (this.activeImgTag + 1) % 2; // change this.activeImgTag

    var img = document.getElementById(
      this.eid + "_img_" + (this.activeImgTag + 1),
    );
    ImageBox.removeEListener(img, "load", this.eventImageLoaded);
    this.justLoadingImage = -1;
