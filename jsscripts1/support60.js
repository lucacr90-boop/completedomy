...
function reload_parent(inserted_id) {
  ...
  if (my_form["parent_form"].value) {
    ...
    if (parent_form["table"])
      parent_form["table"].value = parent_form["self_table"].value;
    if (parent_form["id"])
      parent_form["id"].value = parent_form["self_id"].value;
    ...
    if (my_form["parent_s"].value != "") {
      // novinka:
      window.opener.location.href =
        "?s=" +
        my_form["parent_s"].value +
        (my_form["parent_ss"] && my_form["parent_ss"].value
          ? "&ss=" + my_form["parent_ss"].value
          : "");
    } else {
      // puvodne:
...
function time_reload(time_to_close, atable, countdown) {
  if (
    ...
    frames["iframe_checker"].document.forms["form_checker"]["not_logged_in"]
      .value == "1"
  )
    location.href = "./"; // zrejme uprava aplikace - smazan autentifikacni retezec z db - skok na Login
  if (time_reload_stop_flag) {
    document.getElementById("timereload_headtext").innerHTML =
      '<font color="red">Synchronizace je vypnuta.</font>';
    //		frames['iframe_checker'].location.href = 'index_checker.php?stop=1';  // zastav checker
    //		return false;  // konec odpocitavani, zadny reload
  }
  if (!countdown) {
    // faze porovnavani verze vypsane tabulky a tabulky v databazi
    if (frames["iframe_checker"].document.forms["form_checker"]) {
      // je iframe nacten?
      if (
        frames["iframe_checker"].document.forms["form_checker"][
          "table_version_db_" + atable
        ].value ==
        document.forms["form_writedown"]["table_version_" + atable].value
      ) {
        // tabulka se v db nezmenila
        setTimeout(
          "time_reload(" + time_to_close + ", '" + atable + "', false);",
          1000,
        ); // dalsi kontrola za nejaky cas
        return false;
      } else {
        // tabulka se v db zmenila!
        frames["iframe_checker"].location.href =
          "index_checker.php?stop=1&beep=1"; // zastav checker a zapipej
        document.body.style.backgroundColor = "#f0f0f0";
        if (!time_reload_stop_flag) {
          // pokud je synchronizace off, stopne se time_reload()ovani
          setTimeout(
            "time_reload(" + time_to_close + ", '" + atable + "', true);",
            100,
          ); // spust odpocet
        }
        return false;
      ...
      // iframe 'checker' se zrejme zrovna reloaduje, pockame...
    ...
    // faze odpocitavani casu do reloadu stranky
    document.getElementById("timereload_headtext").innerHTML =
      "Stránka se přenačte za " + time_to_close + " sekund.";
    if (time_to_close > 0) {
      time_to_close--;
      setTimeout(
        "time_reload(" + time_to_close + ", '" + atable + "', true);",
      ...
      window.location.href =
        "./?s=writedown&table=&" +
        (document.forms["form_writedown"]["writedownall"].value == "1"
          ? "writedownall=1"
          : "timereload=1"); // okamzity reload
    }
  }
}
...
function SubmitListing(aform, atable, listingtable, thefirst, parents) {
  if (
    thefirst >= 0 &&
    document.forms[aform]["writedown_listing_first_" + listingtable]
  ) {
    document.forms[aform]["writedown_listing_first_" + listingtable].value =
      thefirst;
  }
  return SubmitForm(
    document.forms[aform]["self_s"].value,
    aform,
    atable,
    document.forms[aform]["self_id"].value,
    parents,
    true,
  );
}
...
function SubmitOrdering(aform, atable, orderby_idx, direction_idx, parents) {
  document.forms[aform]["razeni_" + atable + "_sloupec"].selectedIndex =
    orderby_idx;
  document.forms[aform]["razeni_" + atable + "_smer"].selectedIndex =
    direction_idx;
  return SubmitForm(
    document.forms[aform]["self_s"].value,
    aform,
    document.forms[aform]["self_table"].value,
    document.forms[aform]["self_id"].value,
    parents,
    true,
  );
...
function PrintTableItem(table, id, rest_params) {
  okno = window.open(
    "index_print.php?table=" +
      table +
      "&id=" +
      id +
      (rest_params != "" ? "&" + rest_params : ""),
    "print",
    "left=5,top=5,scrollbars=yes,width=830,height=590,resizable=yes,menubar=yes",
  );
  okno.focus();
  return false;
}
...
function SubmitSetMail(aform, anid, predmet, id_prijemce, parents) {
  ...
  if (id_prijemce !== false) {
    document.getElementById("secondaryedit_uzivatele_vyber_000").name =
      "secondaryedit_uzivatele_vyber_" + id_prijemce;
    document.getElementById("secondaryedit_uzivatele_vyber_000").value = "1";
  }
  return SubmitForm(
    anid == "0" ? "new_mail" : "posta",
    aform,
    "zpravy",
    anid,
    parents,
...
function SubmitShowWin(href) {
  ...
  odoc.getElementById("body").innerHTML =
    '<p style="text-align: center; padding: 20px 20px 30px 20px;"><b>Nahrávám data...</b></p>';
  BodyResized();
  ResizeDisabler(true);
  o.src = href;
  return false;
...
function SubmitShowDetail(atable, anid, rest_params) {
  ...
  o.src =
    "./?s=detail&show_content_only=1&table=" +
    atable +
    "&id=" +
    anid +
    (rest_params !== undefined ? "&" + rest_params : "");
  return false;
}
function SubmitShowDetailZpravy(anid, tr_element) {
  tr_element.className = tr_element.className.replace(
    "tr_zprava_nova",
    "tr_zprava_prectena",
  );
...
function ParentDetailOnLoad() {
  if (window.parent) window.parent.DetailOnLoad();
}
...
function ResizeDetailIframe(show) {
  ...
  var odoc = o.contentDocument ? o.contentDocument : o.contentWindow.document;
  var width = Math.min(
    detail_iframe_max_width,
    Math.round(winw * detail_iframe_max_width_coef),
  );
  ...
  if (html_tags.length == 1) {
    ...
    if (temp_patt.test(html_tags[0].style.width)) {
      var width_preset = parseInt(
        html_tags[0].style.width.substr(0, html_tags[0].style.width.length - 2),
      );
      if (width_preset > 0) {
  ...
  var height = Math.max(
    Math.min(odoc.getElementById("body").scrollHeight, winh * 0.9),
    10,
  );
  ...
  oclose.style.left =
    Math.round(docleft + (winw + width) / 2 + /* 1 */ -65) + "px";
...
function FillInputNow(aform, aninput) {
  ...
  if (document.forms[aform][aninput + "__actual"].checked) {
    document.forms[aform][aninput + "__old_value"].value =
      document.forms[aform][aninput].value;
...
function OpenCalendar(aform, aninput, table, column, cas) {
  if (
    document.forms[aform][aninput + "__actual"] &&
    document.forms[aform][aninput + "__actual"].checked == true
  ) {
    document.forms[aform][aninput + "__actual"].checked = false;
    FillInputNow(aform, aninput);
  }
  okno = window.open(
    "index_kalendar.php?form=" +
      ...
      document.forms[aform][aninput].value,
    "kalendar",
    "left=400,top=100,scrollbars=yes,width=415,height=415,resizable=yes",
  );
  okno.focus();
  return false;
}
...
function WriteInputCasDo(aform, aninput, datum, hod, min) {
  ...
  window.opener.document.forms[aform][aninput].value =
    datum +
    " " +
    ((hod < 10 ? "0" : "") + hod) +
    ":" +
...
function ProcessBgFaders() {
  for (var i = 0; i < bgfaders.length; i++) {
    ...
    if (e_d < 0 && bgfaders[i][3] > 0) {
      ...
      var temp_r = Math.round(
        bgfaders[i][5][0] +
          bgfaders[i][1] * (bgfaders[i][4][0] - bgfaders[i][5][0]),
      );
      var temp_g = Math.round(
        bgfaders[i][5][1] +
          bgfaders[i][1] * (bgfaders[i][4][1] - bgfaders[i][5][1]),
      );
      var temp_b = Math.round(
        bgfaders[i][5][2] +
          bgfaders[i][1] * (bgfaders[i][4][2] - bgfaders[i][5][2]),
      );
      var temp_a =
        bgfaders[i][5][3] +
        bgfaders[i][1] * (bgfaders[i][4][3] - bgfaders[i][5][3]);
      if (rgba_supported) {
        e.style.backgroundColor =
          "rgba(" + temp_r + "," + temp_g + "," + temp_b + "," + temp_a + ")";
      } else {
        e.style.backgroundColor =
          "rgb(" +
          Math.max(temp_r, Math.round((1 - temp_a) * 255)) +
          "," +
          Math.max(temp_g, Math.round((1 - temp_a) * 255)) +
          "," +
          Math.max(temp_b, Math.round((1 - temp_a) * 255)) +
          ")";
      }
    }
  }
...
function PushOrChangeMotionFader(
  eid,
  fade_delta,
  target_x,
  target_y,
...
****/
    ...
    e.style.transform =
      ...
      Math.round(
        (mofaders[i][3][2] +
          ((1 - Math.cos(Math.pow(mofaders[i][1], 1) * Math.PI)) / 2) *
            (mofaders[i][3][5] - mofaders[i][3][2])) *
          10000,
      ) /
        10000 +
      ") translate(" +
...
function ShowOrHideElement(
  ...
  if (command == "show") {
    if (!fade) {
      ...
      PushOrChangeFader(
        element_id,
        fade_len ? fade_len : fade_delta_in,
        no_z_index,
        display_mode,
      );
    }
    ...
    if (!fade) {
      ...
      PushOrChangeFader(
        element_id,
        fade_len ? -fade_len : -fade_delta_out,
        no_z_index,
        display_mode,
      );
    }
...
*/
  ...
  if (advUpload_filelistBoxes) {
    ...
    advUpload_filelistBoxes.forEach(
      // forEach is used because creates new context and we need this to addEventListener('drop', ...)
      function (advUpload_filelistBox) {
        var elements_postfix =
          advUpload_filelistBox[0] +
          advUpload_filelistBox[1] +
          "_" +
          advUpload_filelistBox[2] +
          "_" +
          advUpload_filelistBox[3];
        var box_show = null;
        if (adv_mode) {
          var e_dropArea = document.getElementById(
            "file_upload_drop_area_" + elements_postfix,
          );
          if (e_dropArea) {
            e_dropArea.addEventListener("dragover", AdvUpload_DragOver, false);
            e_dropArea.addEventListener(
              "dragenter",
              AdvUpload_DragEnter,
              false,
            );
          ...
          box_show = document.getElementById(
            "file_upload_advanced_box_" + elements_postfix,
          );
        } else {
          box_show = document.getElementById(
            "file_upload_inputs_box_" + elements_postfix,
          );
        }
...
function PlgAnimNext() {
  ...
  if (e_bgu2) {
    ...
    e_bgu2.style.transform =
      "translate(-50%, -50%) rotate(" + plg_anim_a + "deg)";
...
function ShowNextContentBannerImage() {
  if (content_banner_actual_image) {
    var e_cbi_last = document.getElementById(
      "content_banner_img_" + content_banner_actual_image,
    );
    ElementRemoveClass(e_cbi_last, "content_banner_img_active");
  }

  content_banner_actual_image++;
  var e_cbi = document.getElementById(
    "content_banner_img_" + content_banner_actual_image,
  );
  if (!e_cbi) {
    content_banner_actual_image = 1;
    e_cbi = document.getElementById(
      "content_banner_img_" + content_banner_actual_image,
    );
  }
...
function TransformHeadImage(raf_time_actual) {
  if (ctx_hi) {
    ...
    if (
      /*x head_image_scale < 150 */ last_head_image_alpha > 0 &&
      last_head_image >= 0
    ) {
      var sx =
        Math.pow(head_image_scale, 1) + last_head_image_scale; /*F + hi_width */
      var sy = Math.round((sx * hi_height) / hi_width);
      ...
      last_head_image_alpha =
        (1 + Math.cos((head_image_scale / 40) * Math.PI)) / 2;
      ctx_hi.globalAlpha = last_head_image_alpha;

      ctx_hi.drawImage(
        head_images_data[last_head_image],
        sx,
        sy,
        hi_width - 2 * sx,
        hi_height - 2 * sy,
...
*/
    ...
    if (hi_use_raf) {
      ...
      setTimeout(
        "TransformHeadImage(0);",
        32 - Math.min(31, Math.max(0, time_actual - time_start)),
      ); //R
    }
...
function BublinaBoxMouseOut(e, eid_pf) {
  ...
  PushOrChangeBgFader(
    "bublina_box_bottom_line_" + eid_pf,
    -fade_delta_out,
    1,
    0,
  );
...
function AdminMenuRedraw(menumousex) {
  ...
  if (document.getElementsByTagName) {
    ...
    for (var i = 0; i < tags.length; i++) {
      ...
      var img_rx_dx =
        -(menumousex - img_rx > 0 ? 1 : -1) *
        Math.pow(Math.abs(menumousex - img_rx) / img_size, 0.6) /* *5 */ *
        0;
      if (menumousex !== null) {
      ...
      var img_w =
        ...
        (img_size / 7) *
          (menumousex === null
            ? 1
            : Math.pow(
                Math.min(1, Math.abs(menumousex - img_rx) / img_size / 2),
                1,
              ));
...
function formatNumber(number, precision) {
  ...
  for (var i = 0; i < num_int.length; i++) {
    result =
      num_int.charAt(num_int.length - i - 1) +
      (i % 3 == 0 && i > 0 && num_int.charAt(num_int.length - i - 1) != "-"
        ? " "
        : "") +
      result;
  }
...
function ShowImageMenu(image_menu) {
  if (image_menu_actual != "") {
    ...
    if (e_tr1 && e_tr2) {
      // nemusi existovat, napr. po odstraneni obrazku ajaxem
  ...
  if (image_menu != "") {
    ...
    document.getElementById(image_menu + "_tr1").style.backgroundColor =
      "#49a3da";
    document.getElementById(image_menu + "_tr2").style.backgroundColor =
      "#49a3da";
  }
...
function TMCEInsertLink(id_textarea, file_href, link_text, ev) {
  ...
  aNode.innerHTML =
    '<img src="style/icons/file_icon_2.png" style="max-height: 2em; margin: 0 0.5em;" />' +
    aNode.innerHTML;
...
function TMCEInsertVideo(id_textarea, eid_postfix) {
  ...
  if (video_code_matches) {
    ...
    aNode.setAttribute(
      "src",
      "https://www.youtube.com/embed/" + video_code_matches[1],
    );
...
function AjaxXMLDocStateChange() {
  if (ajax_xmlhttp.readyState == 4) {
    if (ajax_xmlhttp.status == 200) {
      ...
      var x_scdr =
        ajax_xmlhttp.responseXML.documentElement.getElementsByTagName(
          "scd_results",
        );
      if (x_scdr.length > 0) {
        if (x_scdr[0].getAttribute("correct") == "1") {
          ...
          for (var i = 0; i < global_id_role_all.length; i++) {
            if (
              ElementHasClass(
                e_strom,
                "strom_enable_cbox_group_" + global_id_role_all[i],
              )
            ) {
              strom_opened_roles +=
                (strom_opened_roles != "" ? "," : "") + global_id_role_all[i];
            }
          }
          setTimeout(
            "location.href = '?s=strom&strom_opened_roles=" +
              strom_opened_roles +
              "&strom_opened_dirs=" +
              strom_opened_dirs.join(",") +
              "'",
            2000,
          );
        } else {
          setTimeout(
            "HideDisablerTransfer(); ElementRemoveClass(document.getElementById('strom_changes_send_button'), 'strom_changes_send_button_disabled');",
            2000,
          );
        }
      }
      ...
      AddAjaxLog(
        "Nastala chyba při komunikaci se serverem.<br/>Server communication error.",
      );
    }
    ajax_xmlhttp = null;
    ...
    if (x_grafy[i].getAttribute("type") == "Playground") {
      ...
      for (var j = 0; j < x_image.length; j++) {
        if (x_image[j].getAttribute("type") == "Graf") {
          e_grafy_max_zindex =
            Number(x_image[j].getAttribute("z")) > e_grafy_max_zindex
              ? Number(x_image[j].getAttribute("z"))
              : e_grafy_max_zindex;
        }
      }
      ...
      for (var j = 0; j < x_image.length; j++) {
        ...
        if (x_image[j]["type"] == "Graf") {
          ...
          e_graf.style.zIndex =
            e_grafy_max_zindex - x_image[j].getAttribute("z");
          if (x_image[j]["style"].length) {
            e_graf.classList.add(x_image[j]["style"]);
          }
          // read XML - hodnoty
          if (x_image[j]["hodnoty"][0]) {
            var hodnoty_arr = GetSerializedXml(
              x_image[j]["hodnoty"][0].firstChild,
            )
              .toString()
              .split(" ");
            ...
            for (var k = 0; k < hodnotyX.length; k++) {
              ...
              if (x_image[j]["style"] == "bars") {
                ...
                var e_bar_total_width =
                  hodnotyX[k] * unitSizeX + (bar_3d ? k * barSizeZ : 0);
                e_graf_max_width = e_bar_total_width;
                e_graf_max_height =
                  e_bar_height > e_graf_max_height
                    ? e_bar_height
                    : e_graf_max_height;
                ...
                e_bar.style.background = hexToRgbA(
                  "#" + x_image[j].getAttribute("color"),
                  bar_opacity,
                );
                ...
                e_bar_val.style.marginTop =
                  -(bar_3d ? 2.75 : 1.75) * barSizeZ + "px";
                ...
                if (bar_3d) {
                  ...
                  e_bar_top.style.background = hexToRgbA(
                    "#" + x_image[j].getAttribute("color"),
                    bar_opacity - bar_opacity_3d_step,
                  );
                  ...
                  e_bar_left.style.background = hexToRgbA(
                    "#" + x_image[j].getAttribute("color"),
                    bar_opacity + bar_opacity_3d_step,
                  );
          ...
          e_grafy_max_height =
            e_graf_max_height > e_grafy_max_height
              ? e_graf_max_height
              : e_grafy_max_height;
          e_grafy_max_unitSizeX =
            unitSizeX > e_grafy_max_unitSizeX
              ? unitSizeX
              : e_grafy_max_unitSizeX;
          e_grafy_max_width =
            e_graf_max_width > e_grafy_max_width
              ? e_graf_max_width
              : e_grafy_max_width;

          if (x_image[j]["style"] == "bars") {
            e_grafy_max_barSizeX =
              barSizeX > e_grafy_max_barSizeX ? barSizeX : e_grafy_max_barSizeX;
            e_grafy_max_barSizeZ =
              barSizeZ > e_grafy_max_barSizeZ ? barSizeZ : e_grafy_max_barSizeZ;
          ...
          e_after_grafy.innerHTML +=
            "graf[" +
            x_image[j]["style"] +
            "]" +
            GetSerializedXml(x_image[j]["hodnoty"][0]) +
            "<br/>";
        }
      } // /cyklus graf (images)
      ...
      e_grafy.style.height =
        parseInt(
          e_grafy_max_height +
            (bar_3d ? e_grafy_graf_of_bars_count * e_grafy_max_barSizeZ : 0),
        ) + "px";
...
function AddAjaxLogs(ajax_xmlhttp) {
  var e_logs =
    ajax_xmlhttp.responseXML.documentElement.getElementsByTagName("log");
  var time = 0;
...
function AddAjaxLog(log, time) {
  ...
  o_ajax_logs.innerHTML +=
    '<div id="ajax_log_' +
    ajax_logs_counter +
    '" class="ajax_log"><img src="style/icons/cancel_grey_24.png" onclick="return HideElement(\'ajax_log_' +
    ajax_logs_counter +
    '\');" class="ajax_log_close_button" title="Zavřít" />' +
    log +
    "</div>";
...
function FMS_GetNotePocet(pocet) {
  return pocet && pocet != "0"
    ? "(" + pocet + ")"
    : '<span class="form_select_item_note_zero">0</span>';
}
...
function FMS_AddItem(
  ...
  var subitem_eid =
    subitem_e === true ? true : subitem_e ? subitem_e + "_" + subitem_id : "";
  var out_eid = subitem_eid /* && subitem_eid !== true */ ? item_eid : item_e;

  FMS_HideOptions(
    "fmsi_" + (subitem_eid && subitem_eid !== true ? item_eid : item_e),
  );
  ...
  if (e_hint_set) {
    ...
    var e_item = document.getElementById(
      "fmsi_" + item_eid + (subitem_eid ? "_" + subitem_eid : ""),
    );
    var e_item_note = document.getElementById(
      "fmsi_" + item_eid + (subitem_eid ? "_" + subitem_eid : "") + "_note",
    );
    var chosen_fid =
      "fmsi_" + item_eid + (subitem_eid ? "_" + subitem_eid : "") + "_chosen";
    e_hint_set.innerHTML =
      '<span id="' +
      chosen_fid +
      '_note" class="form_select_item_note">' +
      (e_item_note
        ? /* e_item_note.childNodes[0].nodeValue */ e_item_note.innerHTML
        : "") +
      "</span>" +
      e_item.childNodes[1].innerHTML;
    HideElement("fmsi_" + out_eid + "_hint");
    ShowElement("fmsi_" + out_eid + "_hint_set");
    //x		ShowElement('fmsi_'+out_eid+'_cancel');
  } else if (
    !FMS_GetFilter(out_eid).length &&
    document.getElementById("fmsi_" + out_eid + "_hint2") &&
    subitem_e !== true
  ) {
    HideElement("fmsi_" + out_eid + "_hint");
  ...
  if (!single && !text_item) {
    HideElement(
      "fmsi_" +
        item_eid +
        (subitem_eid && subitem_e !== true ? "_" + subitem_eid : ""),
    );
  }
    ...
    if (!document.getElementById(chosen_fid)) {
      ...
      if (e_chosens) {
        e_chosens.innerHTML +=
          '<div id="' +
          chosen_fid +
          '" class="form_select_subitem"><input type="checkbox" checked="checked" onchange="FMS_DropItem(\'' +
          item_e +
          "', '" +
          item_id +
          "'" +
          (subitem_eid ? ", '" + subitem_e + "', '" + subitem_id + "'" : "") +
          ');" title="odebrat" /><span id="' +
          chosen_fid +
          '_note" class="form_select_item_note">' +
          (e_item_note
            ? /* e_item_note.childNodes[0].nodeValue */ e_item_note.innerHTML
          ...
          "</span>" +
          e_item.childNodes[1].innerHTML +
          "</div>"; //  + '<span class="form_select_item_drop" onclick="...">x</span>'
  ...
  if (!dont_load) {
    FMS_Load(
      "a=add_item," +
        item_eid +
        (subitem_eid && subitem_eid !== true ? "," + subitem_eid : ""),
      false,
    );
  }
...
function FMS_DropItem(
  ...
  if (e_hint_set) {
    ...
    if (subitem_eid !== true) {
      var e_chosen = document.getElementById(
        "fmsi_" + item_eid + (subitem_eid ? "_" + subitem_eid : "") + "_chosen",
      );
      if (e_chosen) {
        var e_chosens = document.getElementById("fmsi_" + out_eid + "_chosens");
...
x*/
  ...
  if (
    !document.getElementById("global_page_is_homepage") &&
    !dont_include_filters
  ) {
    var e_input = document.getElementById("fmsi_nazev");
    if (e_input) {
      var f = FMS_GetFilter("nazev");
      if (
        (f === false && e_input.value != "") ||
        (f !== false && e_input.value != f)
      ) {
  ...
  if (dont_change_hash !== true) {
    ...
    window.location.hash =
      "#fmsi=" + actual_filters + (params != "" ? "&" + params : "");
  }
  if (params != "") {
...
function WindowHashChanged() {
  ...
  if (
    location.hash != "" &&
    (hash_regs = /(^|&|#)fmsi=(.*?)($|&)/.exec(location.hash))
  ) {
    fmsi = hash_regs[2];
  } else if (
    location.search != "" &&
    (search_regs = /(^|&|\?)fmsi=(.*?)($|&)/.exec(location.search))
  ) {
    fmsi = search_regs[2];
  }
  ...
  if (
    location.hash != "" &&
    (hash_regs = /(^|&|#)vo_id=([1-9][0-9]*?)($|&)/.exec(location.hash))
  ) {
    vo_id = hash_regs[2];
  } else if (
    location.search != "" &&
    (search_regs = /(^|&|\?)vo_id=([1-9][0-9]*?)($|&)/.exec(location.search))
  ) {
    vo_id = search_regs[2];
  }
  ...
  if (
    location.hash != "" &&
    (hash_regs = /(^|&|#)list=([1-9][0-9]*?)($|&)/.exec(location.hash))
  ) {
    list = hash_regs[2];
  } else if (
    location.search != "" &&
    (search_regs = /(^|&|\?)list=([1-9][0-9]*?)($|&)/.exec(location.search))
  ) {
    list = search_regs[2];
  }
  ...
  if (
    location.hash != "" &&
    (hash_regs = /(^|&|#)vo_show_next=([1-9][0-9]*?)($|&)/.exec(location.hash))
  ) {
    vo_show_next = true;
  } else if (
    location.search != "" &&
    (search_regs = /(^|&|\?)vo_show_next=1($|&)/.exec(location.search))
  ) {
    vo_show_next = true;
  }
  ...
  if (
    location.hash != "" &&
    (hash_regs = /(^|&|#)vo_show_prev=([1-9][0-9]*?)($|&)/.exec(location.hash))
  ) {
    vo_show_prev = true;
  } else if (
    location.search != "" &&
    (search_regs = /(^|&|\?)vo_show_prev=1($|&)/.exec(location.search))
  ) {
    vo_show_prev = true;
  }
  ...
  FMS_Load(
    "a=get_all_boxes&fmsi=" +
      fmsi +
      (vo_id != "" ? "&vo_id=" + vo_id : "") +
      (list != "" ? "&list=" + list : "") +
      (vo_show_next ? "&vo_show_next=1" : "") +
      (vo_show_prev ? "&vo_show_prev=1" : "") +
      (document.getElementById("global_page_is_homepage")
        ? "&page_is_homepage=1"
        : ""),
    false /*true ... go_top*/,
...
function FE_ItemChanged(e) {
  AjaxLoad([
    "fe_item_changed=" + e.name + "&" + "fe_item_changed_val=" + e.value,
  ]);
}
...
function ElementHasClass(e, c) {
  if (e) {
    return e.className
      ? e.className.match(new RegExp("([ ]|^)" + c + "([ ]|$)"))
      : false;
  } else {
    return null;
  }
...
function EscapeHTML(htmltext) {
  ...
  return htmltext.replace(/[&<>"']/g, function (ch) {
    return char_map[ch];
  });
}
...
function UnEscapeHTML(htmltext) {
  ...
  htmltext = htmltext.replace(/&lt;|&gt;|&quot;|&#039;/g, function (ch) {
    return char_map[ch];
  });
...
function AdvUpload_SetProgressBar(
  progress_elements_postfix,
  message_text,
  val,
) {
  var e_progress_message = document.getElementById(
    "progress_message_" + progress_elements_postfix,
  );
  var e_progress_bar = document.getElementById(
    "progress_bar_" + progress_elements_postfix,
  );
  var e_progress_bar_text = document.getElementById(
    "progress_bar_text_" + progress_elements_postfix,
  );
...
function AdvUpload_FileListChanged(
  fileinput,
  item_table,
  item_id,
  item_column,
...
function AdvUpload_UploadNext() {
  ...
  if (advUpload_uploadList.length) {
    ...
    if (e_prog_top_text) {
      e_prog_top_text.innerHTML =
        "Probíhá odesílání dat.<br/>Zbývá souborů: <b>" +
        advUpload_uploadList.length +
        "</b>";
    }
    ...
    if (e_prog_top_text) {
      e_prog_top_text.innerHTML =
        '<span class="zelene_pismo">Přenos dat ukončen.</span>';
    }
...
function AdvUpload_ResizeAndDoUpload(file, item_table, item_id, item_column) {
  ...
  var file_type =
    0o0 && file.type.match("image/(jpeg|png|gif)$") ? "image" : "file"; ///// 000 - zde zakazano, nechceme zmensovat ani prevadet png na jpeg atp.
  //-			console.log('UPLOADING ' + file.name);
  var progress_elements_postfix =
    item_table +
    "_" +
    item_id +
    "_" +
    item_column +
    "_" +
    ++advUpload_progressBoxCounter;
  var progress_elements_short_postfix =
    item_table + "_" + item_id + "_" + item_column;
  ...
  var e_progresses_inner = document.getElementById(
    "progresses_inner_" + progress_elements_short_postfix,
  );
  e_progresses_inner.innerHTML =
    '<div class="progress_container"><div id="progress_thumbnail_' +
    progress_elements_postfix +
    '" class="progress_thumbnail"></div><div class="progress_filename">Odesílání <b>' +
    EscapeHTML(file.name) +
    '</b></div><div id="progress_message_' +
    progress_elements_postfix +
    '" class="progress_message"></div><div class="progress_bar_outer"><div id="progress_bar_text_' +
    progress_elements_postfix +
    '" class="progress_bar_text"></div><div id="progress_bar_' +
    progress_elements_postfix +
    '" class="progress_bar"></div></div><div class="progress_container_bottom"></div></div>' +
    e_progresses_inner.innerHTML;

  if (file_type == "image") {
    AdvUpload_SetProgressBar(
      progress_elements_postfix,
      "Zpracovávám obrázek...",
      0,
    );
    ...
    o_img.onerror = function () {
      AdvUpload_SetProgressBar(
        progress_elements_postfix,
        '<span class="cervene_pismo">Během práce s obrázkem nastala chyba - zřejmě neznámý formát.</span>',
        null,
      );
    ...
    o_img.onabort = function () {
      AdvUpload_SetProgressBar(
        progress_elements_postfix,
        '<span class="cervene_pismo">Během práce s obrázkem nastala chyba - přenos přerušen.</span>',
        null,
      );
      AdvUpload_UploadNext(); // ?!
    };
    o_img.onload = function () {
      ...
      if (img_width > 0 && img_height > 0) {
        ...
        if (img_width > 0 && img_height > 0) {
          ...
          var o_thumb = document.getElementById(
            "progress_thumbnail_" + progress_elements_postfix,
          );
          if (o_thumb) {
          ...
          xhr.onreadystatechange = function (e) {
            if (xhr.readyState == 4) {
              if (xhr.status == 200) {
                AdvUpload_SetProgressBar(
                  progress_elements_postfix,
                  '<span class="zelene_pismo">Úspěšně odesláno.</span>',
                  100,
                );
                var e_progress_results = document.getElementById(
                  "progress_results_" + progress_elements_short_postfix,
                );
                var x_pr =
                ...
                if (
                  ...
                  e_progress_results.innerHTML = GetSerializedXml(
                    x_pr[0].firstChild,
                  );
                  ImageBox.createAll(
                    "progress_results_" + progress_elements_short_postfix,
                ...
                AddAjaxLog(
                  "Nastala chyba při komunikaci se serverem - status " +
                    xhr.status +
                    ".",
                );
              }
          ...
          var e_prv = document.getElementById(
            "progress_results_view_" + progress_elements_short_postfix,
          );
          ...
          var data =
            ...
            item_column +
            (e_prv && e_prv.value != ""
              ? "&result_view=" + encodeURIComponent(e_prv.value)
              : "") +
            "&filename_orig=" +
          ...
          AdvUpload_SetProgressBar(
            progress_elements_postfix,
            '<span class="cervene_pismo">Obrázek má po zmenšení nulovou velikost.</span>',
            "",
          );
          AdvUpload_UploadNext();
        }
        ...
        AdvUpload_SetProgressBar(
          progress_elements_postfix,
          '<span class="cervene_pismo">Obrázek má nulovou velikost nebo není obrázkem.</span>',
          "",
        );
        AdvUpload_UploadNext();
      }
...
function AdvUpload_ChangeImagePosition(
  item_table,
  item_id,
  direction,
  progress_results_element,
  disk_id_adresar,
) {
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function (e) {
      ...
      if (xhr.status == 200) {
        if (disk_id_adresar) {
          ...
          if (
            x_pr &&
            x_pr[0] &&
            x_pr[0].getAttribute("disk_id_adresar") !== null
          ) {
            location.href =
              "?s=disk&id_adresar=" + x_pr[0].getAttribute("disk_id_adresar");
          }
        } else {
          var e_progress_results = document.getElementById(
            progress_results_element,
  ...
  xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  var data =
    "s=ajax&change_item_position=1&table=" +
    item_table +
    "&id=" +
    item_id +
    "&direction=" +
    encodeURIComponent(direction) +
    (disk_id_adresar ? "&disk_id_adresar=" + disk_id_adresar : "");
  xhr.send(data);
...
function PushOrChangeScrollFader(eid, eid2) {
  ...
  var params = [
    e.scrollLeft,
    e.scrollTop,
    e2_rect.left - e_rect.left,
    Math.max(
      0,
      Math.min(
        e.scrollHeight - e.clientHeight,
        e.scrollTop - (e_rect.top - e2_rect.top),
      ),
    ),
  ];
...
function ProcessScrollFaders() {
  for (var i = 0; i < scfaders.length; i++) {
    ...
    e.scrollLeft = Math.round(
      scfaders[i][3][0] +
        ((1 - Math.cos(Math.pow(scfaders[i][1], 0.5) * Math.PI)) / 2) *
          (scfaders[i][3][2] - scfaders[i][3][0]),
    );
    e.scrollTop = Math.round(
      scfaders[i][3][1] +
        ((1 - Math.cos(Math.pow(scfaders[i][1], 0.5) * Math.PI)) / 2) *
          (scfaders[i][3][3] - scfaders[i][3][1]),
    );
  }
}
...
function DocumentScrollTo(new_x, new_y) {
  ...
  var new_scroll_x = Math.round(
    document_scroll_to_x +
      (actual_scroll[0] - document_scroll_to_x) / document_scroll_to_coef,
  );
  var new_scroll_y = Math.round(
    document_scroll_to_y +
      (actual_scroll[1] - document_scroll_to_y) / document_scroll_to_coef,
  );
  window.scrollTo(new_scroll_x, new_scroll_y);
...
function CalendarGoToPg(pg) {
  if (calendar_act_pg) {
    ElementRemoveClass(
      document.getElementById("calendar_box_pg_" + calendar_act_pg),
...
function RefreshCalendar() {
  ...
  var datum_regs =
    /^[^0-9]*([0-9]{1,2})[^0-9]+([0-9]{1,2})[^0-9]+([0-9]{4})/.exec(
      calendar_act_input_element.value + "",
    );
  if (datum_regs) {
    ...
    var datum_regs = /^[^0-9]*([0-9]{1,2})[^0-9]+([0-9]{4})/.exec(
      calendar_act_input_element.value + "",
    );
    if (datum_regs) {
...
function RedrawCalendar() {
  ...
  if (calendar_act_pg == 3) {
    var temp_roky = document
      .getElementById("calendar_years_list")
      .value.split(",");
    ...
    PushOrChangeScrollFader(
      "calendar_months",
      "calendar_month_" + calendar_act_month,
    );
  }
}
...
function CalendarClick(e) {
  if (ElementHasClass(e, "calendar_td_active")) {
    calendar_act_input_element.value =
      e.innerText + ". " + calendar_act_month + ". " + calendar_act_year;
    HideCalendar();
    //x		CPSubmitForm();
  }
}
...
function CalendarClickMonth(e) {
  calendar_act_input_element.value =
    calendar_act_month + ". " + calendar_act_year;
  HideCalendar();
  //x	CPSubmitForm();
}
...
function js_CopyElementHTMLToClipboard(ev, e_name) {
  ...
  if (!result) {
    alert(
      "Chyba: Váš prohlížeč zřejmě nepodporuje funkci kopírování HTML do schránky.",
    );
  }
    ...
    if (ElementHasClass(trs[i], "tr_default")) {
      ...
      for (var j = 0; j < tds.length; j++) {
        if (!ElementHasClass(tds[j], "icons")) {
          ...
          if (
            tds[j].hasChildNodes() &&
            ElementHasClass(tds[j].firstChild, "export_data_text")
          ) {
      ...
      if (!head_exported) {
        // muze byt vice radku hlavicky, proto zde
        head_exported = true;
      }
    } else if (!head_exported) {
      // pocitame s tim, ze tabulka zacina hlavickou
...
function KA_CalendarGoToPg(pg) {
  if (ka_calendar_act_pg) {
    ElementRemoveClass(
      document.getElementById("ka_calendar_box_pg_" + ka_calendar_act_pg),
      "calendar_box_pg_active",
    );
  }
...
function KA_RedrawCalendar() {
  ...
  if (ka_calendar_act_pg == 3) {
    var temp_roky = document
      .getElementById("ka_calendar_years_list")
      .value.split(",");
    for (i = 0; i < temp_roky.length; i++) {
    ...
    for (i = 1; i <= 12; i++) {
      ...
      if (
        i == ka_calendar_chosen_month &&
        ka_calendar_act_year == ka_calendar_chosen_year
      ) {
        ElementAddClass(e, "calendar_td_selected");
      } else {
        ElementRemoveClass(e, "calendar_td_selected");
    ...
    while (o_datum.getMonth() + 1 == ka_calendar_act_month) {
      ...
      e.innerHTML =
        d +
        '<div id="ka_calendar_akce_' +
        ka_calendar_act_year +
        "_" +
        ka_calendar_act_month +
        "_" +
        d +
        '" class="ka_calendar_akce" onmouseover="ShowElement(\'ka_calendar_akce_' +
        ka_calendar_act_year +
        "_" +
        ka_calendar_act_month +
        "_" +
        d +
        "_details', false, true);\" onmouseout=\"HideElement('ka_calendar_akce_" +
        ka_calendar_act_year +
        "_" +
        ka_calendar_act_month +
        "_" +
        d +
        "_details', false, true);\"></div>";
      if (111 || o_datum.getTime() >= o_dnes.getTime()) {
        ElementAddClass(e, "calendar_td_active");
      } else {
        ElementRemoveClass(e, "calendar_td_active");
      ...
      if (
        d == ka_calendar_chosen_day &&
        ka_calendar_act_month == ka_calendar_chosen_month &&
        ka_calendar_act_year == ka_calendar_chosen_year
      ) {
        ElementAddClass(e, "calendar_td_selected");
      } else {
    ...
    AjaxLoad([
      "ka_load=1",
      "ka_year=" + ka_calendar_act_year,
      "ka_month=" + ka_calendar_act_month,
    ]);
  }
}
...
function ShowHpMenu() {
  ...
  while (document.getElementById("hp_menu_itembox_" + i)) {
    setTimeout(
      "ElementAddClass(document.getElementById('hp_menu_itembox_" +
        i +
